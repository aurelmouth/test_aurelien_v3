...
function barecu(id){
	console.log(id);
	$('#ba_recu #num_particulier').html(function(){
		return id;
	});
	$('#ba_recu #ba_recu_particulier').attr('href', '<?php echo Router::url('receptionba/barecu/');?>' + id);
	$("#ba_recu").modal('show');
}
...
<?php if($this->request->action == 'create'){?>
$('input[name="sFirstName"], input[name="sLastName"]').on('keyup keypress blur change paste cut', function(){
